// Copyright Communication Protocols Team 2022 (skel)
...
/** Copyright 2021 Communication Prootocols Team
 * I took these functions from the last year's skel, which was present (by
 * mistake) in the first skel we received from the PCom Team. The functions
 * are not written by myself and I do not claim them as being myselves. 
 */
...
void send_icmp_error(uint32_t daddr, uint32_t saddr, uint8_t *sha, uint8_t *dha, u_int8_t type, u_int8_t code, int interface, void *info)
{
	...
	memcpy(packet.payload + packet.len, info, 64 * sizeof(uint8_t));
	packet.len += 64 * sizeof(uint8_t);
